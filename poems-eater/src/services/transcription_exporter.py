...
class TranscriptionExporter:
    ...
    def __init__(self, transcriptions_dir: str = "../poems-eater/transcriptions"):
    ...
    def export_to_excel(
        ...
        for idx, poem in enumerate(poems, 1):
            if poem.disponibilidad in ["ENCONTRADO", "PARCIAL"]:
                audio_filename = f"poems-eater_{poem.numero:03d}_{poem.titulo.replace(' ', '_')[:50]}.m4a"
                
                transcription = self.load_transcription(audio_filename)
                
                if transcription:
                    poem.transcripcion = transcription
                    stats['with_transcription'] += 1
                    print(f"[{idx}/{stats['total']}] {poem.titulo[:50]} - Transcripción cargada ({len(transcription)} chars)")
                else:
                    stats['without_transcription'] += 1
                    print(f"[{idx}/{stats['total']}] {poem.titulo[:50]} - No se encontró transcripción")
            else:
                stats['no_audio'] += 1
        
        df = pd.DataFrame([poem.to_dict() for poem in poems])
    ...
    def print_statistics(self, stats: Dict[str, int]):
        ...
        print(f"{'='*60}")
        print(f"   Total de poemas: {stats['total']}")
