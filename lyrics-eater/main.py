...
class LyricsEaterRunner(BaseEaterRunner):
    ...
    def __init__(self):
        super().__init__(
            module_name="lyrics",
            title="Lyrics Eater - Dominican Songs Processor"
        )
    ...
    def get_service(self, client: Tuple[GeniusSearcherAdapter, YouTubeSearcherAdapter]) -> LyricsService:
    ...
    def load_input_data(self) -> List[Any]:
        ...
        if not searches:
                ...
                logger.info(" Tip: Create '{self.config.SEARCHES_FILE}' with one search per line")
            return []
