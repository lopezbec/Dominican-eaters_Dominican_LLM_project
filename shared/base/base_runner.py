...
class BaseEaterRunner(ABC):
    ...
    def load_from_excel(self, excel_path: str) -> List[Any]:
        """
        Load items from existing Excel file.
        
        This allows resuming/updating previous results without re-searching.
        ...
        try:
            ...
            if df.empty:
                logger.info(f"Excel file is empty: {excel_path}")
                return []
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    item = model_class.from_dict(item_dict)
                    items.append(item)
                except Exception as e:
                    logger.warning(f"Failed to load row {idx}: {e}")
                    continue
            
            logger.info(f"Loaded {len(items)} items from Excel: {excel_path}")
            return items
            
        except ImportError:
            logger.error("pandas not available - cannot load from Excel")
            return []
        except Exception as e:
    ...
    def save_dual_format(self, items: List[Any]) -> bool:
        ...
        if valid_items:
            excel_success = FileHandler.save_to_excel(
                valid_items, self._get_output_file()
            )
